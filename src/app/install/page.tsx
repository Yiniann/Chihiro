...
export default async function InstallPage() {
  ...
  const siteSettings =
    installationState.status === "ready" ? await getSiteSettings() : null;
    ...
    <main className="mx-auto min-h-screen w-full max-w-4xl px-6 py-16 sm:px-10">
      <section className="max-w-3xl">
        ...
        <p className="mt-4 text-base leading-8 text-zinc-500 dark:text-zinc-400">
          先把数据库和表结构准备好，再在这里完成站点信息和首个管理员初始化。
        </p>
      </section>
      ...
      <section className="mt-10 rounded-[2rem] border border-zinc-200/80 bg-white/85 p-6 shadow-[0_24px_70px_rgba(24,24,27,0.08)] dark:border-zinc-800/80 dark:bg-zinc-950/55 dark:shadow-[0_24px_80px_rgba(0,0,0,0.28)] sm:p-8">
        {installationState.status === "missing_database" ? (
          <InstallHint
            title="当前还不能开始初始化"
            description="站点还没有完成安装，而且当前环境里也没有可用的数据库连接串。先配置 DATABASE_URL 并重启应用，再回来继续初始化。"
            helper={
              <code className="break-all rounded-xl bg-zinc-100 px-3 py-2 text-xs text-zinc-700 dark:bg-zinc-900 dark:text-zinc-300">
                DATABASE_URL=postgresql://user:password@host:5432/chihiro?schema=public
              </code>
            }
          />
        ) : installationState.status === "database_unavailable" ? (
          <InstallHint
            title="当前还不能继续初始化"
            description="站点还没有完成安装。虽然数据库连接串已经配置，但数据库目前不可用，所以现在还不能继续写入初始化数据。请先检查数据库实例、网络连通性、连接串和凭据。"
          />
