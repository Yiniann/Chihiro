...
export function StandalonePageActionMenu({
  standalonePageId,
  isPublished,
  editHref,
  compact = false,
}: StandalonePageActionMenuProps) {
      ...
      <button
        ...
        className={
          compact
            ...
            : "inline-flex items-center gap-1.5 border-b border-transparent px-0 py-1 text-xs font-medium text-zinc-500 transition hover:border-zinc-300 hover:text-zinc-900 dark:text-zinc-400 dark:hover:border-zinc-700 dark:hover:text-zinc-100"
        }
        aria-label="更多操作"
      >
        {compact ? (
      ...
      {isOpen ? (
        <div className="absolute right-0 z-10 mt-2 min-w-[8rem] overflow-hidden rounded-2xl border border-zinc-200/90 bg-white p-1 shadow-[0_14px_40px_rgba(15,23,42,0.08)] dark:border-zinc-800/90 dark:bg-zinc-950 dark:shadow-[0_14px_40px_rgba(0,0,0,0.35)]">
          ...
          {isPublished ? (
            ...
            <form action={publishStandalonePageAction}>
              <input type="hidden" name="id" value={standalonePageId} />
              <button
                type="submit"
